...
public class DateChooserJButton extends JButton {
	...
	boolean homeDateFlag;
	String title = "日期时间选择";

	/**
	 * 标记是否是首页的日期选择器 该选择器会触发数值的重新计算
	 * 
	 * @param homeDateFlag
	 */
	public DateChooserJButton(boolean homeDateFlag, String title) {
		this(getNowDate());
		if (!"".equals(title)) {
			this.title = title;
		}
		this.homeDateFlag = homeDateFlag;
	}

	public DateChooserJButton() {
		this(getNowDate());
		this.homeDateFlag = false;
	}
	 ...
	 *            标题
	 */
	public DateChooserJButton(String dateString, String title) {
		this();
		if (!"".equals(title)) {
			this.title = title;
		}
	...
	public DateChooserJButton(Date date, String title) {
		this("", date);
		if (!"".equals(title)) {
			this.title = title;
		}
	...
	private class DateChooser extends JPanel implements ActionListener, ChangeListener {
		...
		private JDialog createDialog(Frame owner) {
			JDialog result = new JDialog(owner, title, true);
		...
		void showDateChooser(Point position) {
			if (SwingUtilities.getWindowAncestor(DateChooserJButton.this) instanceof Frame) {
				Frame owner = (Frame) SwingUtilities.getWindowAncestor(DateChooserJButton.this);
				if (dialog == null || dialog.getOwner() != owner) {
					dialog = createDialog(owner);
				}
				dialog.setLocation(getAppropriateLocation(owner, position));
			} else if (SwingUtilities.getWindowAncestor(DateChooserJButton.this) instanceof JDialog) {
		...
		private void dayColorUpdate(boolean isOldDay) {
			...
			if (isOldDay) {
				...
				daysButton[i][j].setContentAreaFilled(false);
			} else {
				daysButton[i][j].setForeground(todayBackColor);
				daysButton[i][j].setContentAreaFilled(true);
			}
		}
	...
	public static void main(String[] args) {
		...
		mainFrame.add(new DateChooserJButton("666 ", new Date()), java.awt.BorderLayout.CENTER);
